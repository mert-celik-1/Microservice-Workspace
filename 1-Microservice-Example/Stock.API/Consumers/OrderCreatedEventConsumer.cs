...
namespace Stock.API.Consumers
{
    ...
    {
        IMongoCollection<Stock.API.Models.Stock> _stockCollection;
        ...
        {
            _stockCollection = mongoDBService.GetCollection<Stock.API.Models.Stock>();
        ...
        public async Task Consume(ConsumeContext<OrderCreatedEvent> context)
        {
            ...
            if (stockResult.TrueForAll(sr => sr.Equals(true)))
            {
                foreach (OrderItemMessage orderItem in context.Message.OrderItems)
                {
                    Stock.API.Models.Stock stock = await (await _stockCollection.FindAsync(s => s.ProductId == orderItem.ProductId)).FirstOrDefaultAsync();
